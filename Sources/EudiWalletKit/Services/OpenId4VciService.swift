...
public actor OpenId4VciService {
	...
	func initSecurityKeys(_ configuration: CredentialConfiguration, proofIssuer: String) async throws -> ([BindingKey], [Data]) {
		...
		let bindingKeys = try publicKeys.enumerated().map { try createBindingKey($0.element, secureAreaSigningAlg: selectedAlgorithm, unlockData: unlockData, index: $0.offset, funcKeyAttestationJWT: funcKeyAttestationJWT, proofIssuer: proofIssuer) }
	...
	func createBindingKey(_ publicKeyJWK: ECPublicKey, secureAreaSigningAlg: MdocDataModel18013.SigningAlgorithm, unlockData: Data?, index: Int, funcKeyAttestationJWT: FuncKeyAttestationJWT?, proofIssuer: String) throws -> BindingKey {
		...
		if funcKeyAttestationJWT == nil {
			bindingKey = .jwt(algorithm: JWSAlgorithm(algType), jwk: publicKeyJWK, privateKey: .custom(signer), issuer: proofIssuer)
		} else {
			bindingKey = try! .jwtKeyAttestation(algorithm: JWSAlgorithm(algType), keyAttestationJWT: funcKeyAttestationJWT!, keyIndex: UInt(index), privateKey: .custom(signer), issuer: proofIssuer)
		}
		return bindingKey
	}
	...
	func issueDocumentsByOfferUrl(offerUri: String, docTypes: [OfferedDocModel], authorized: AuthorizedRequest?, forceRefreshToken: Bool = false, documentId: String?, txCodeValue: String? = nil, promptMessage: String? = nil, backgroundOnly: Bool = false, dpopKeyId: String? = nil) async throws -> [WalletStorage.Document] {
		...
		let proofIssuer = await issuer.config.client.id
		let issuerName = offer.credentialIssuerMetadata.display.map(\.displayMetadata).getName(uiCulture) ?? offer.credentialIssuerIdentifier.url.host ?? offer.credentialIssuerIdentifier.url.absoluteString
		...
		let documents = try await withThrowingTaskGroup(of: WalletStorage.Document.self) { group in
			for (i, openId4VCIService) in openId4VCIServices.enumerated() {
				group.addTask {
					let (bindingKeys, publicKeys) = try await openId4VCIService.initSecurityKeys(credentialInfos[i], proofIssuer: proofIssuer)
	...
	func resumePendingIssuance(pendingDoc: WalletStorage.Document, webUrl: URL?) async throws -> IssuanceOutcome {
		...
		let (bindingKeys, publicKeys) = try await initSecurityKeys(model.configuration, proofIssuer: await issuer.config.client.id)
