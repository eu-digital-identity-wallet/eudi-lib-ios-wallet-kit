...
public final class EudiWallet: ObservableObject {
    ...
    func prepareIssuing(id: String, docType: String?, displayName: String?, keyOptions: KeyOptions?, disablePrompt: Bool, promptMessage: String? = nil) async throws -> (IssueRequest, OpenId4VCIService, String) {
    ...
    @discardableResult public func issuePAR(docType: String, format: DataFormat = .cbor, keyOptions: KeyOptions? = nil, promptMessage: String? = nil) async throws -> WalletStorage.Document {
        let (issueReq, openId4VCIService, id) = try await prepareIssuing(id: UUID().uuidString, docType: docType, displayName: nil, keyOptions: keyOptions, disablePrompt: false, promptMessage: promptMessage)
        ...
        return try await finalizeIssuing(data: data, docType: docType, format: format, issueReq: issueReq, openId4VCIService: openId4VCIService)
    }
    ...
    @discardableResult public func resumePendingIssuance(pendingDoc: WalletStorage.Document, webUrl: URL?, keyOptions: KeyOptions? = nil) async throws -> WalletStorage.Document {
        ...
        let outcome = try await openId4VCIService.1.resumePendingIssuance(pendingDoc: pendingDoc, webUrl: webUrl)
        ...
        let res = try await finalizeIssuing(data: outcome, docType: pendingDoc.docType, format: format, issueReq: openId4VCIService.0, openId4VCIService: openId4VCIService.1)
        return res
    }
    ...
    public func resolveOfferUrlDocTypes(uriOffer: String, format: DataFormat = .cbor) async throws -> OfferedIssuanceModel {
        ...
        return try await openId4VCIService.1.resolveOfferDocTypes(uriOffer: uriOffer, format: format)
    }
    ...
    public func issueDocumentsByOfferUrl(offerUri: String, docTypes: [OfferedDocModel], docTypeKeyOptions: [String: KeyOptions]? = nil, txCodeValue: String? = nil, format: DataFormat = .cbor, promptMessage: String? = nil, claimSet: ClaimSet? = nil) async throws -> [WalletStorage.Document] {
        ...
        for (i, docTypeModel) in docTypes.enumerated() {
            ...
            guard let docData = try await openId4VCIService.1.issueDocumentByOfferUrl(offerUri: offerUri, docTypeModel: docTypeModel, txCodeValue: txCodeValue, format: format, promptMessage: promptMessage, claimSet: claimSet) else { continue }
            
            let docType = docTypes[i].docType
            let data = try await openId4VCIService.1.issuePAR(docType: docType, format: format, promptMessage: "")
            
            
            documents.append(try await finalizeIssuing(data: data, docType: docType, format: format, issueReq: openId4VCIService.0, openId4VCIService: openId4VCIService.1))
        }
    ...
    func finalizeIssuingCborDocument(id: String, data: Data, docType: String, format: DataFormat, issueReq: IssueRequest, openId4VCIService: OpenId4VCIService) async throws -> WalletStorage.Document  {
        ...
        issued = WalletStorage.Document(docType: docTypeToSave, docDataType: ddt, data: dataToSave, secureAreaName: issueReq.secureAreaName, createdAt: Date(), displayName: nil, status: .issued)
...
//        }
    ...
    public func getCredentials(docType: String, dpopNonce: String, code: String, keyOptions: KeyOptions? = nil) async throws -> WalletStorage.Document {
        let (issueReq, openId4VCIService, id) = try await prepareIssuing(id: UUID().uuidString, docType: docType, displayName: nil, keyOptions: keyOptions, disablePrompt: false, promptMessage: nil)
